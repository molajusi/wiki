...
본 문서는 메트로이드배니아 지식 클러스터의 **전문 분과 3: 컨트롤러 입력 설계 및 글로벌 표준화** 독립 지식 문서입니다. 메트로이드배니아뿐만 아니라 소울라이크, FPS/TPS 전반을 아우르는 게임 인터페이스 단독 표준 규격서이며, 전체 장르 총론은 상위 메인 허브 [메트로이드배니아 장르 개요 및 계보학](metroidvania_genre_analysis.html)에서 확인할 수 있습니다.
</context>
...
본 문서는 사쿠라이 마사히로가 자신의 유튜브 채널에서 실제로 밝힌 조작 설계 철학(2026-08-28 갱신: 아래 참조)과 글로벌 게임 스튜디오들의 역사적 시행착오를 바탕으로, 복잡한 입력을 직관적으로 조직하는 **5대 입력 설계 기법**과 **글로벌 컨트롤러 매핑 표준**을 체계화합니다. 다만 5대 기법 전부가 사쿠라이 본인의 발언에서 직접 도출된 것은 아니며, 각 기법의 출처는 개별 각주로 구분해 표기합니다.
</overview>
...
- **4단계 (2010년대~현재): 프롬 소프트웨어의 '우측 숄더 공격(R1/R2)'과 엄지손가락 해방(Right Thumb Liberation)**:
  ...
  - <b>용어 출처 정정(2026-08-28)</b>: "엄지손가락 해방(Right Thumb Liberation)"은 미야자키 히데타카 본인이나 프롬 소프트웨어의 공식 발언에서 확인된 용어가 아닙니다. R1/R2 공격 배치와 오른쪽 스틱 자유화 사이의 인과관계를 설명하기 위해 이 위키가 자체적으로 붙인 분석적 명칭이며, 이를 뒷받침하는 개발자 인터뷰는 검색으로 확인되지 않았습니다. 버튼 배치 자체(R1=약공격 등)는 실제 게임 사실이지만, "왜 이렇게 설계했는가"에 대한 개발자 본인의 설명으로 오인되지 않도록 명시합니다.

## 4. 시스템 버튼군의 40년 진화와 '시스템 좌/우 버튼'의 이원화 철학
...
- **엄지손가락 해방**: **Right Thumb Liberation**. 기본 공격과 가드를 숄더/트리거(R1/R2)에 배정하여 공격 중에도 우측 엄지손가락이 카메라 제어 스틱에서 떨어지지 않게 만든 조작 배치를 가리키는, 이 위키가 붙인 분석적 명칭. 미야자키 히데타카나 프롬 소프트웨어의 공식 발언으로 확인된 용어는 아님(§3 4단계 참조).
...
- **로컬 원천 데이터**: [`raw/20260822_metroidvania_genre_analysis_raw.txt`](file:///home/molajusi/home-nas/wiki/raw/20260822_metroidvania_genre_analysis_raw.txt), [`raw/20260828_game_controller_sakurai_citation_correction_raw.txt`](raw/20260828_game_controller_sakurai_citation_correction_raw.txt)(2026-08-28 각주 정정 경위)
- **본문 인용 및 출처 각주 목록 (Numbered References)**:
  1. [21] **Masahiro Sakurai (2022~2024)**. *Masahiro Sakurai on Creating Games* (유튜브 채널 — 단일 영상이 아니라 채널 전반에 걸쳐 반복 언급되는 주제). 조작 접근성과 숨겨진 복잡성을 병행하는 설계 철학("한 버튼과 방향키만으로도 플레이 가능하게" 등)을 확인. [웹링크](https://www.youtube.com/@sora_sakurai_en) (2026-08-28 정정: 기존에 단일 영상 제목처럼 표기됐던 것을 채널·주제 단위 인용으로 수정)
